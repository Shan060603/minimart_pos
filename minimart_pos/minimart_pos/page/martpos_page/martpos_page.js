...
class MiniMartPOS {
    ...
    bind_events() {
        ...
        $(document).on('click', (e) => {
            if (!$(e.target).closest('#barcode-scan, #item-group-filter, #customer-search-container, .awesomplete, .modal-dialog, .cart-qty-input, .btn-qty').length) {
                this.focus_input();
            }
        });
    ...
    process_payment() {
        let me = this;
        let total = flt(this.$total_display.text());
        if (!this.cart.length) return;

        let d = new frappe.ui.Dialog({
            title: __('Finalize Payment'),
            fields: [
                { label: __('Payable'), fieldname: 'total', fieldtype: 'Currency', default: total, read_only: 1 },
                { label: __('Method'), fieldname: 'mop', fieldtype: 'Select', options: me.shift_data.payment_methods || ['Cash'], default: 'Cash' },
                { 
                    label: __('Received'), fieldname: 'received', fieldtype: 'Currency', default: total,
                    onchange: function() {
                        let change = flt(this.get_value()) - total;
                        d.get_field('change_display').$wrapper.find('.change-val').text('₱' + (change >= 0 ? change.toFixed(2) : '0.00'));
                    }
                },
                { fieldtype: 'HTML', fieldname: 'change_display', options: `<div class="text-right mt-2"><span class="change-val" style="font-size: 1.8rem; font-weight: bold; color: #27ae60;">₱0.00</span></div>` }
            ],
            primary_action_label: __('Complete'),
            primary_action(values) {
                frappe.call({
                    method: "minimart_pos.api.create_invoice",
                    args: { cart: JSON.stringify(me.cart), customer: me.customer_control.get_value(), mode_of_payment: values.mop, amount_paid: values.received },
                    freeze: true,
                    callback: (r) => {
                        me.trigger_cash_drawer();
                        d.hide();
                        me.print_receipt(r.message, [...me.cart], total, flt(values.received), flt(values.received) - total);
                        me.cart = [];
                        me.render_cart();
        ...
        d.show();
        setTimeout(() => d.get_field('received').$input.select(), 400);
    }

    print_receipt(name, cart, total, paid, change) {
        let print_window = window.open('', 'PRINT', 'height=600,width=400');
        let receipt_html = `<html><body onload="window.print(); window.close();"><center><h3>${this.shift_data.company}</h3><p>RECEIPT: ${name}</p></center><hr>
            <table>${cart.map(i => `<tr><td>${i.item_name}</td><td align="right">${i.qty} x ${i.price.toFixed(2)}</td></tr>`).join('')}</table><hr>
            <div style="display:flex; justify-content:space-between;"><b>TOTAL:</b><b>₱${total.toFixed(2)}</b></div></body></html>`;
